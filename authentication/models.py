...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, email, first_name, last_name,sap_id, password):
        user = self.create_user(email, first_name, last_name, sap_id, password)
        user.staff = True
        user.admin = True
        user.save(using=self._db)
        return user
...
class User(AbstractBaseUser):
    ...
    REQUIRED_FIELDS = ['first_name', 'last_name', 'sap_id']
